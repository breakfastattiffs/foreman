class Host::Managed < Host::Base
  ...
  def host_inherited_params include_source = false
    ...
    domain.domain_parameters.each {|p| hp.update Hash[p.name => include_source ? {:value => p.value, :source => N_("domain(#{domain.name})").to_sym} : p.value] } unless domain.nil?
    # read OS parameters
    operatingsystem.os_parameters.each {|p| hp.update Hash[p.name => include_source ? {:value => p.value, :source => N_("os(#{operatingsystem.to_label})").to_sym} : p.value] } unless operatingsystem.nil?
